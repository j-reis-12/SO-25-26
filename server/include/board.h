...
typedef struct {
    ...
    pthread_t *ghost_tids; // ghost threads for this board
    int ready; // start sending the updated board when true
    int victory; // level finished
    int game_over; // dead pacman
    int points;
    int shutdown;
} board_t;
